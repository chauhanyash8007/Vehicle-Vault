...
export default function VehicleCard({ vehicle, onCompareToggle, selected, onFavorite }) {
  const imageUrl = vehicle.images?.[0] || "https://via.placeholder.com/400x220?text=No+Image";
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl">
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-600">
          <div className="rounded-lg bg-slate-50 p-2">Price: Rs. {vehicle.price}</div>
          <div className="rounded-lg bg-slate-50 p-2">Mileage: {vehicle.mileage} km/l</div>
          <div className="rounded-lg bg-slate-50 p-2">Engine: {vehicle.engine || "N/A"}</div>
          <div className="rounded-lg bg-slate-50 p-2">Trans: {vehicle.transmission || "N/A"}</div>
        </div>
          ...
          {onCompareToggle && (
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-medium ${
                selected ? "bg-brand-600 text-white" : "border border-slate-300 text-slate-700 hover:bg-slate-100"
              }`}
            >
